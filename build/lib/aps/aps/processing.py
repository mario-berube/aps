...
from aps.utils import readDICT, saveDICT, app
...
class Processing:
    ...
    def __init__(self, vgosdb, session, ac_code='NASA'):
    ...
    def check_agency(self):
        return self.session.analysis_center.upper() == self.ac_code
    ...
    def init_required(self):
        info = toml.load(app.get_hidden_file('aps.toml').open(), _dict=dict)
        label = info['Title']
