...
class STDreport:
    ...
    def write_comments(self, title, comments, in_line=None, sched=None, vgosdb=None, spool=None):
...
class INTreport(STDreport):
    ...
    def write_comments(self, title, comments, in_line=None, sched=None, vgosdb=None, spool=None):
        ...
        if vgosdb and spool:
            self.write_missed_obs(sched, vgosdb, spool, in_line)

    def write_missed_obs(self, sched, vgosdb, spool, in_line):
        # Write rejected obs at correlation
        not_corr = self.get_correlated_data(sched, vgosdb)
        unknown = '?'*len(str(vgosdb.correlated))
        not_corr_str = str(len(not_corr)) if sched else unknown
        recovered = self.get_total_recoverable(sched, vgosdb, spool)
        ...
        unusable = vgosdb.correlated - recovered
        ...
        if lst := spool.unused.get('unusable', []):
            for obs in lst:
                self.write(obs)
            self.write('')
        elif unusable > 0:
        ...
        if lst := spool.unused.get('excluded', []):
            for obs in lst:
                self.write(obs)
            self.write('')
        elif excluded > 0:
