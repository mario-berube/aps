...
class Section:
    ...
    def __init__(self, spl):
        ...
        self.header = {}
        self.read_all()
        self.MJD_NUT = self.MJD_NUT if self.MJD_NUT else self.MJD_EOP
...
# Check if SPOOL_DIR is in environment
def get_spool_folder():
    if folder := os.environ.get('SPOOL_DIR'):
        return folder

    with open(app.Applications.APS.get('set_csolve', '/sgpvlbi/progs/set_csolve.csh')) as f:
        for line in f:
            if 'SPOOL_DIR' in line:
                os.environ['SPOOL_DIR'] = folder = line.split()[-1]
                return folder
    return None
...
def read_spool(path):
    if not os.path.exists(path):
        path = os.path.join(get_spool_folder(), f'SPLF{path}')
