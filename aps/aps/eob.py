...
from aps.aps.leap_seconds import get_UTC_minus_TAI
from aps.aps.astro import MSEC__TO__RAD, OM__EAR
...
# List describing order and format of output (see eops-format.txt)
...
def eob_to_eops(path_eob, path_eops):
    # Read eops-format.txt file
    eops_format = [x if x.startswith('#') else f'# {x}' for x
                   in open(app.get_hidden_file('eops-format.txt')).readlines()]
