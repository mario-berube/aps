...
from aps.utils.files import remove
from aps.aps import solve
from aps.aps.spool import read_spool
from aps.aps.eop import EOP
from aps.aps.eob import eob_to_eops
...
class EOPM(EOP):
    ...
    def __init__(self, opa_config, initials, run_simul=False):
        ...
        self.sites, self.min_dist = {}, 0

        self.check_required_files(['EOPT_FILE', 'EOPM_FILE'], chk_write=True)

        path = Path(getattr(app, 'sites', os.path.join(os.environ.get('MODEL_DIR', '/'), 'glo.sit')))
        if path.exists():
            self.min = int(getattr(app, 'min_dist', '5000000'))
            with open(path) as sit:
